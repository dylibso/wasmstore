...
let rec read_file buf ic f : unit Lwt.t =
...
let file_stream filename : _ Lwt_stream.t =
  Lwt_eio.run_lwt @@ fun () ->
...
let stdin_stream () : _ Lwt_stream.t =
  Lwt_eio.run_lwt @@ fun () ->
  let buf = Bytes.create buf_size in
  let s, push = Lwt_stream.create () in
...
let commit =
  let cmd store hash =
    ...
    let fail body = Printf.fprintf stderr "ERROR %s" body in
    match hash' with
...
let audit =
  let cmd store path =
    ...
    let t = store env in
    let lm =
      Lwt_eio.run_lwt @@ fun () -> Store.last_modified (Wasmstore.store t) path
    in
    List.iter
      (fun commit ->
